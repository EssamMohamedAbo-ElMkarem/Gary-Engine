...
from Gary.hub.auth import Auth
from Gary.hub.session import HubTrainingSession
from Gary.hub.utils import PREFIX, split_key
from Gary.yolo.utils import LOGGER, emojis, is_colab
from Gary.yolo.utils.torch_utils import select_device
from Gary.yolo.v8.detect import DetectionTrainer
...
def start(key=''):
    # Start training models with Gary HUB. Usage: from src.Gary import start; start('API_KEY')
    def request_api_key(attempts=0):
        ...
        input_key = getpass.getpass("Enter your Gary HUB API key:\n")
...
def reset_model(key=''):
    ...
    api_key, model_id = split_key(key)
    r = requests.post('https://api.Gary.com/model-reset', json={"apiKey": api_key, "modelId": model_id})
...
def get_export(key='', format='torchscript'):
    ...
    r = requests.post('https://api.Gary.com/get-export',
                      json={
                          "apiKey": api_key,
                          "modelId": model_id,
                          "format": format})
