...
from Gary.nn.autobackend import AutoBackend
from Gary.yolo.configs import get_config
from Gary.yolo.data.dataloaders.stream_loaders import LoadImages, LoadScreenshots, LoadStreams
from Gary.yolo.data.utils import IMG_FORMATS, VID_FORMATS
from Gary.yolo.utils import DEFAULT_CONFIG, LOGGER, SETTINGS, callbacks, colorstr, ops
from Gary.yolo.utils.checks import check_file, check_imgsz, check_imshow
from Gary.yolo.utils.files import increment_path
from Gary.yolo.utils.torch_utils import select_device, smart_inference_mode
from Gary.garySE.cfg import config

Cls2inx = config.Class2Index
...
class BasePredictor:
    ...
    def __init__(self, config=DEFAULT_CONFIG , overrides=None):
        ...
        self.class_names = list(self.classes.split("::"))
        self.classes = []
        for class_ in self.class_names[:-1]:
            self.classes.append(int(Cls2inx[class_]))
    ...
    def setup(self, source=None, model=None):
        ...
        if webcam:
            ...
            self.dataset = LoadImages(source, rate = self.rate, trim = self.trim, start = self.start,
            end = self.end ,imgsz=imgsz, stride=stride, auto=pt, vid_stride=self.args.vid_stride)
            # print(self.dataset)
            # print("Karim")
            # print(next(iter(self.dataset)))
    ...
    def __call__(self, source=None, model=None):
        ...
        for batch in self.dataset:
            ...
            path, im, im0s, vid_cap, s, flag = batch
            # print("kkkkk")
            # print(batch)
