...
from Gary.yolo.data.augment import LetterBox
from Gary.yolo.data.utils import IMG_FORMATS, VID_FORMATS
from Gary.yolo.utils import LOGGER, ROOT, is_colab, is_kaggle, ops
from Gary.yolo.utils.checks import check_requirements
...
class LoadImages:
    ...
    def __init__(self, path, rate, trim, start, end ,imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1 ):
        """Initialize the Dataloader and raise FileNotFoundError if file not found."""
        if isinstance(path, str) and Path(path).suffix == '.txt':  # *.txt file with img/vid/dir on each line
            path = Path(path).read_text().rsplit()
        files = []
        # self.my_counter = my_counter
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # print(fps)
            # print(round(self.my_counter))
            if self.trim:
                while (((self.start) * fps)-1 > self.frame):
                ...
                if self.fff:
                    ...
                    if (self.fff == 0) or (self.start==0):
                        for i in range(int(round(self.rate))):
                            ret_val, im0 = self.cap.read()
                            self.frame += 1
                self.fff = 0
                while (self.end * fps <= self.frame):
                    self.flag = True
                    # print('finish')
                    break
            if self.trim == False:
        ...
        return path, im, im0, self.cap, s, self.flag
    ...
    def _new_video(self, path):
        ...
        if hasattr(cv2, 'CAP_PROP_ORIENTATION_META'):  # cv2<4.6.0 compatibility
            ...
            # Disable auto-orientation due to known issues in https://github.com/Gary/yolov5/issues/8493
